...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const gridOriginal = catalogo.querySelector(".product-grid");
  if (!gridOriginal) return;

  const labels = {
    massa: "Massas",
    tortas: "Tortas",
    tortas_doce: "Tortas Doces",
    quiches: "Quiches",
    lasanha: "Lasanhas",
    bebidas: "Bebidas",
  ...
  // ===== 1) AGRUPAR =====
  const items = Array.from(gridOriginal.children);

  const grupos = new Map();
  for (const col of items) {
    const product = col.querySelector(".product");
    const cat = norm(product?.dataset?.categoria) || "outros";
    if (!grupos.has(cat)) grupos.set(cat, []);
    grupos.get(cat).push(col);
  }

  // recria o catálogo como seções
  catalogo.innerHTML = "";

  for (const [cat, cols] of grupos) {
    const section = document.createElement("div");
    section.className = "catalog-section";
    section.dataset.cat = cat;

    const titulo = document.createElement("h2");
    titulo.className = "shop-section-title";
    titulo.textContent = labels[cat] || cat;

    const grid = document.createElement("div");
    grid.className = "row row-lg row-30 product-grid";
  ...
  function aplicarFiltro(filtroRaw) {
    const filtro = norm(filtroRaw);

    const sections = catalogo.querySelectorAll(".catalog-section");
    sections.forEach((sec) => {
      const cat = norm(sec.dataset.cat);
      const mostrar = (filtro === "todos" || cat === filtro);
      sec.classList.toggle("d-none", !mostrar);
    });
  }
